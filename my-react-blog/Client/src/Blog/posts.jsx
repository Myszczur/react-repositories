...
function Posts() {
    ...
    useEffect(() => {
        if (!context.postsState) {
            ...
            if (context.postsState && !stateLocal.fetched) {
                ...
                const last_page = Math.cell(context.postsState.length / stateLocal.posts_per_page);
    ...
    }, [stateLocal, stateLocal.currentPage]);
    ...
    return (
        <>
            <div style={{ opacity: stateLocal.opacity, transition: 'opacity 2s ease' }}>
                <br />
                {context.authState ?
                    <Link to='/addpost'><Button variant='contained' color='primary'>Add Post</Button></Link>
                    : <Link to='/signup'><Button variant='contained' color='primary'>Sign Up to Add Post</Button></Link>
                }
            </div>
            <br />
            <TextField
            ...
            <div>
                {stateLocal.posts_search
                    ? stateLocal.posts_search.map(post => <RenderPosts key={post.pid + 1000} post={post} />)
                    : null}
            </div>
            <h1>Posts</h1>
            <div>
                {stateLocal.posts_slice
                    ? stateLocal.posts_slice.map(post => <RenderPosts key={post.pid} post={post} />)
                    : null}
            </div>
            <div>
                <div className='FlexRow'>
